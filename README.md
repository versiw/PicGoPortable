...
PicGo Portable 是 PicGo （基于版本：2.4.0-beta.8）的便携版，它是一个绿色免安装版本，可以方便地部署到任何设备上，无需安装即可使用。它包含了 PicGo 的所有功能，并且可以在没有管理员权限的情况下运行。

因为我喜欢将一些工具类软件同步网盘进行使用，但是原本 PicGo 并没有进行用户配置文件和应用缓存的分离，刚好自己最近也在学习 Electron，所以就自己魔改了一些无足轻重的东西，更多的便于自己使用，并且也希望方便大家使用。

在这里，非常感谢 PicGo 作者 `Molunerfinn`的开源贡献！

⚠️⚠️⚠️做了以下几点修改

- 更改了用户配置文件的路径，改为在软件`.exe`可执行文件同级的`.cache`目录下
- 考虑到网盘同步时的特殊情况，将 Electron 应用运行缓存保持在`UserData`目录下
- 更改了 Electron 主进程的窗口可调节，默认窗口大小放大处理
- 相册区域的每行可展示数量提升为 `6`列
- 优化了上传区、相册的UI界面布局 
- 关闭检查更新、更新助手功能
