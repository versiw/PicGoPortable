...
import { PASTE_TEXT, GET_PICBEDS, OPEN_IMAGE_IN_BROWSER } from '#/events/constants'
import { CheckboxValueType, ElMessageBox } from 'element-plus'
import { Close, CaretBottom, Document, Edit, Delete, CaretTop, Link } from '@element-plus/icons-vue'
import {
  ipcRenderer,
  clipboard,
  IpcRendererEvent
...
async function openBrowser (item: ImgInfo) {
  const rest = await ipcRenderer.invoke(OPEN_IMAGE_IN_BROWSER, getRawData(item))
}
